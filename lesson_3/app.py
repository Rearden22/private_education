...
async def main():
    ...
    # await woofi.swap_eth_to_usdt(amount=TokenAmount(amount=0.0001))
    ...
    # await woofi.swap_eth_to_wbtc(amount=TokenAmount(amount=0.001))
    res = await woofi.swap_usdc_to_eth()
    # res = await woofi.swap_arb_to_usdc(slippage=2)
